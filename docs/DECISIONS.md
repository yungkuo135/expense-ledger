...
個人帳號無法申請電子發票 API，因此 macOS 自動化使用獨立 Chrome profile 操作官方
網站。圖形驗證碼與手機條碼登入一律由使用者完成，不保存帳密、不嘗試繞過驗證。
登入後才自動按月份查詢與下載，CSV 必須透過正式網站既有匯入流程，不直接改寫
Supabase 月份 snapshot。

## 信用卡帳單提醒使用獨立清單

每月帳單是否已處理不能只由交易月份或最近匯入批次推測，因此以
`credit-card-import-plan-v1`
獨立保存固定銀行、提醒日與每月完成／略過狀態。信用卡 CSV
匯入仍沿用既有帳務與批次流程；清單只追蹤待辦，不改變交易資料格式。
