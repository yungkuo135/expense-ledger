...
發票匯入完成後會自動顯示「本次新增發票消費」，包含跨月新增項目、發票張數、
品項數及計入支出金額；略過的重複發票不列入。可點開詳情調整分類或分帳，
若有尚未分類的品項，也可直接前往「待處理」的 AI 輔助分類。
按「清除全部」返回完整清單，也可在「匯入 → 最近匯入」點「查看新增消費」
重新開啟尚未復原的批次。此功能也適用手動匯入，沿用既有批次資料，不需 migration。
